...
void disp_spi_add_device_with_speed(spi_host_device_t host, int clock_speed_hz) {
    ...
    spi_device_interface_config_t devcfg = {
            ...
            .flags = SPI_DEVICE_NO_DUMMY |
                     SPI_DEVICE_HALFDUPLEX,    /* dummy bits should be explicitly handled via DISP_SPI_VARIABLE_DUMMY as needed */
    };
    ...
    if (TransactionPool == NULL) {
        ...
        for (size_t i = 0; i < SPI_TRANSACTION_POOL_SIZE; i++) {
            spi_transaction_ext_t *pTransaction = (spi_transaction_ext_t *) heap_caps_malloc(
...
void disp_wait_for_pending_transactions(void) {
    ...
    while (uxQueueMessagesWaiting(TransactionPool) <
           SPI_TRANSACTION_POOL_SIZE) {    /* service until the transaction reuse pool is full again */
        if (spi_device_get_trans_result(spi, &presult, 1) == ESP_OK) {
...
bool lvgl_spi_driver_init() {
    ...
    esp_err_t ret = spi_bus_initialize(TFT_SPI_HOST, &buscfg, SPI_DMA_CH1);
    assert(ret == ESP_OK);
    return ESP_OK != ret;
}